...
std::vector<double> pca::get_record(long record_index) const {
	return utils::extract_row_vector(data_, record_index);
}
...
void pca::set_do_bootstrap(bool do_bootstrap, long number, long seed) {
	if (number < 10){
		throw std::invalid_argument(std::string(__PRETTY_FUNCTION__) + ":Number of bootstraps smaller than ten.");
	}
...
void pca::set_solver(const std::string& solver) {
	if (solver!="standard" && solver!="dc"){
		throw std::invalid_argument(utils::join(std::string(__PRETTY_FUNCTION__) + ":No such solver available: ", solver));
	}
	solver_ = solver;
}
...
void pca::solve() {
	...
	if (num_records_ < 2){
		throw std::logic_error(std::string(__PRETTY_FUNCTION__) + ":Number of records smaller than two.");
	}
...
void pca::set_num_retained(long num_retained) {
	if (num_retained<=0 || num_retained>num_vars_){
		throw std::range_error(utils::join(std::string(__PRETTY_FUNCTION__) + ":Value out of range: ", num_retained));
	}
...
std::vector<double> pca::to_principal_space(const std::vector<double>& data) const {
	...
	return utils::extract_row_vector(row, 0);
}
...
std::vector<double> pca::to_variable_space(const std::vector<double>& data) const {
	...
	column += mean_;
	return utils::extract_column_vector(column, 0);
}
...
std::vector<double> pca::get_energy_boot() const {
	return utils::extract_column_vector(energy_boot_, 0);
}

double pca::get_eigenvalue(long eigen_index) const {
	if (eigen_index >= num_vars_){
		throw std::range_error(utils::join(std::string(__PRETTY_FUNCTION__) + ":Index out of range: ", eigen_index));
	}
	return eigval_(eigen_index);
}

std::vector<double> pca::get_eigenvalues() const {
	return utils::extract_column_vector(eigval_, 0);
}

std::vector<double> pca::get_eigenvalue_boot(long eigen_index) const {
	return utils::extract_column_vector(eigval_boot_, eigen_index);
}

std::vector<double> pca::get_eigenvector(long eigen_index) const {
	return utils::extract_column_vector(eigvec_, eigen_index);
}

std::vector<double> pca::get_principal(long eigen_index) const {
	return utils::extract_column_vector(princomp_, eigen_index);
}
...
double pca::check_projection_accurate() const {
	if (data_.n_cols!=eigvec_.n_cols || data_.n_rows!=princomp_.n_rows){
		throw std::runtime_error(std::string(__PRETTY_FUNCTION__) + ":No proper data matrix present that the projection could be compared with.");
	}
...
std::vector<double> pca::get_mean_values() const {
	return utils::extract_column_vector(mean_, 0);
}

std::vector<double> pca::get_sigma_values() const {
	return utils::extract_column_vector(sigma_, 0);
}
