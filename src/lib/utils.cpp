	...
	if (data.n_cols != means.n_elem) {
		throw std::range_error(
				std::string(__PRETTY_FUNCTION__)
						+ ":Number of elements of means is not equal to the number of columns of data");
	}
...
void normalize_by_column(arma::Mat<double>& data,
	...
	if (data.n_cols != rms.n_elem) {
		throw std::range_error(
				std::string(__PRETTY_FUNCTION__)
						+ ":Number of elements of rms is not equal to the number of columns of data");
	}
	for (uint64_t i = 0; i < data.n_cols; ++i) {
		if (rms(i) == 0) {
...
std::vector<double> extract_column_vector(const arma::Mat<double>& data,
	...
	if (index < 0 || index >= long(data.n_cols)) {
		throw std::range_error(
				join(std::string(__PRETTY_FUNCTION__) + ":Index out of range: ",
						index));
	}
	const long n_rows = data.n_rows;
...
void assert_file_good(const bool& is_file_good, const std::string& filename) {
	if (!is_file_good) {
		throw std::ios_base::failure(
				join(std::string(__PRETTY_FUNCTION__) + ":Cannot open file: ",
						filename));
	}
}
